...
,			int i_num_channels
    ...
    imread( input, image_rows, image_cols,
						num_channels, clean_image );
	if( clean_image == NULL )
    ...
    imread( u_mtx, Uimage_rows, Uimage_cols,
						Unum_channels, U );
	if( U == NULL )
		fail("Unable to get the mask image");
	...
	PLE_sub_routine( noisy, invVar, num_channels, sigma, patch_size, 
					overlap, num_orientations, U, output, 
					epsilon );
...
,			double epsilon
	...
	// Oriented bases:
	// Decide whether to compute the initialization base or load it 
	// from file
	NSym * covs;
	...
	MatrixXd * restored = PLE_denoise( noisy, invVar, 
									overlap, patch_size, 
									num_channels, sigma, num_models, U, 
									new_covs, new_mus, epsilon );
	
	// Write out the images with the required names
	cout << "Writting final image." << endl;
	int num_channels_out = 1;
    imwrite( output, restored, num_channels_out );
...
,		double epsilon
	...
	//convert (mask image will be released in convertFormat)
	...
	//convert (invVar image will be released in convertFormat)
	...
	make_clusters_init( mus, covs, patches_at_row, invVar_at_row, cluster_map, sigma, num_models, num_channels, U_at_row, restored_patches );
