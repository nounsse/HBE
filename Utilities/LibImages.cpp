...
/**
 * @brief Load image a gray pgm image of 16bits.
 *
 * @param p_name : name of the image to read;
 * @param o_im : vector which will contain the image : R, G and B concatenated;
 * @param o_imSize : will contain the size of the image;
...
,   const bool p_verbose
	...
	double *imTmp = NULL;
	int w, h, c = 1;
	imTmp = read_pgm_double(p_name, &w, &h);
	if (!imTmp) {
		cout << "error :: " << p_name << " not found or not a correct png image" << endl;
		return EXIT_FAILURE;
	}
...
,   const bool p_verbose
    ...
    for (unsigned k = 0; k < i_im.size(); k++) {
        ...
        o_imNoisy[k] += p_sigma * (double) (sqrtl(-2.0l * log(a)) * cos(2.0l * M_PI * b));
    }
...
,	const bool p_isForward
	...
	if (p_isForward) {
		if (chnls == 3) {
			...
			const double a			= 1.f / sqrtf(3.f);
			const double b			= 1.f / sqrtf(2.f);
			const double c			= 2.f * a * sqrtf(2.f);
...
,	const unsigned p_nb
	...
	const unsigned wTmp = ceil(double(p_imSize.width)  / double(nW));
	const unsigned hTmp = ceil(double(p_imSize.height) / double(nH));	
	//cout << nW << " " << nH << " " << wTmp << " " << hTmp << " " << p_N << endl;	
	...
	//cout << o_imSizeSub.height << " " << o_imSizeSub.width << endl;
	return EXIT_SUCCESS;
}
...
,	const unsigned p_N
	...
	imSizeTmp.wh        = imSizeTmp.width * imSizeTmp.height;
	imSizeTmp.whc       = imSizeTmp.wh * imSizeTmp.nChannels;
	vector<double> imTmp(imSizeTmp.whc);
